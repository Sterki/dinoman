...
export function AddFoodToMeal({ onAdd, onCancel }: Props) {
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      {!selectedFood ? (
        <>
          <Input label={t('foods.searchFood')} value={search} onChange={e => setSearch(e.target.value)} placeholder={t('foods.searchFoodPlaceholder')} autoFocus />
          <ul className="flex flex-col gap-1 max-h-60 overflow-y-auto">
            {foods.map(food => (
              <li key={food.id}>
                <button onClick={() => setSelectedFood(food)} className="w-full flex items-center justify-between px-4 py-3 rounded-xl text-left hover:bg-blue-50 transition-colors">
                  <span className="font-medium text-slate-800">{food.name}</span>
                  <span className="text-sm text-slate-500">{food.carbsPer100g}{t('common.carbsOf100g')}</span>
                </button>
              </li>
            ))}
            {foods.length === 0 && <p className="text-center text-slate-400 py-6 text-sm">{t('common.noResults')}</p>}
          </ul>
        </>
      ) : (
        <>
          <div className="flex items-center gap-3 p-3 bg-blue-50 rounded-xl">
            ...
            <button onClick={() => { setSelectedFood(null); setGrams('') }} className="text-blue-400 hover:text-blue-600 text-lg">✕</button>
          </div>

          <Input label={t('foods.gramsConsumed')} type="number" min="0.1" step="0.1" value={grams} onChange={e => setGrams(e.target.value)} placeholder={t('foods.gramsConsumedPlaceholder')} autoFocus />
      ...
      <div className="flex gap-3 pt-1">
        ...
        {selectedFood && <Button type="button" fullWidth loading={loading} onClick={handleAdd}>{t('foods.addToMeal')}</Button>}
      </div>
    </div>
  )
}
